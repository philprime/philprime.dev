...
Once validation succeeds, Let's Encrypt issues a certificate and cert-manager stores it in a Secret that Traefik (configured in [Lesson 8](/guides/migrating-k3s-to-rke2-without-downtime/lesson-8)) can use to terminate TLS.
...
Placing it in `/var/lib/rancher/rke2/server/manifests/` ensures it is installed automatically when the cluster starts, following the same pattern used for Longhorn in [Lesson 7](/guides/migrating-k3s-to-rke2-without-downtime/lesson-7) and Traefik in [Lesson 8](/guides/migrating-k3s-to-rke2-without-downtime/lesson-8).
...
Production rate limits are strict — if you hit them during debugging, you may be locked out for a week.
Once certificates issue correctly with staging, switch the annotation on your Ingress to the production issuer.
...
This keeps CRD lifecycle tied to the chart version, so upgrades handle schema changes automatically.
...
Watch the pods come up in the `cert-manager` namespace:
...
RKE2's deploy controller retries failed manifests, so we can place the ClusterIssuers in the same manifests directory — they will fail initially while cert-manager is still starting, then succeed automatically once the CRDs are registered.
...
Verify that all cert-manager pods are running and the webhook is ready:
...
To verify the full chain works — from certificate request through ACME challenge to signed certificate — create a test `Certificate` resource.
This requires a domain name that resolves to your cluster's ingress IP (the Hetzner Load Balancer configured in [Lesson 8](/guides/migrating-k3s-to-rke2-without-downtime/lesson-8)):
...
The following example shows the pattern — you do not need to apply this now, but it demonstrates how workloads will use cert-manager going forward:
