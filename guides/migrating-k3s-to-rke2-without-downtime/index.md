...
| Feature               | Benefit                               |
| --------------------- | ------------------------------------- |
| Single binary (~60MB) | Easy deployment, minimal dependencies |
| SQLite default        | No separate etcd cluster needed       |
| Bundled components    | Traefik, CoreDNS, local-path included |
| ARM support           | Runs on Raspberry Pi and edge devices |
| Low resource usage    | ~512MB RAM for control plane          |
| Quick installation    | Single curl command to deploy         |

### RKE2 Strengths

| Feature                 | Benefit                              |
| ----------------------- | ------------------------------------ |
| FIPS 140-2 compliance   | Required for US government workloads |
| CIS benchmark hardening | Security best practices by default   |
| Embedded etcd           | Production-ready HA storage          |
| Flexible CNI            | Choose your networking solution      |
| Pod Security Standards  | Enforced by default                  |
| SELinux support         | Works with mandatory access controls |
...
flowchart TB
    subgraph k3s["k3s HA (requires external database)"]
        direction TB
        subgraph k3s_nodes[" "]
            direction LR
            K1["Node 1<br/>k3s"]
        ...
        DB[("External<br/>PostgreSQL/MySQL")]
        K1 --> DB
        K2 --> DB
        K3 --> DB
    end

    subgraph rke2["RKE2 HA (embedded etcd)"]
        direction TB
        subgraph rke2_nodes[" "]
            direction LR
            R1["Node 1<br/>RKE2+etcd"]
            R2["Node 2<br/>RKE2+etcd"]
            R3["Node 3<br/>RKE2+etcd"]
        end
        RAFT(["Embedded Raft Consensus"])
        R1 <--> RAFT
        R2 <--> RAFT
        R3 <--> RAFT
    end
