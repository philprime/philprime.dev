...
On top of that, my programming language and framework of choice kept shifting, as I moved on from Java Spring Boot to Nest.js, and rather recently to my new favorite Go, which made me implement health endpoints in different ways and with different libraries.
...
On the other hand, if our health endpoint returns a `warn` status, Kubernetes will still consider the pod as alive but not ready to receive traffic.
...
For this example the overall status is `fail`, because the database check's `fail` status is propagated to the overall status, and the returned HTTP status code will be `503`.
...
As we might have additional checks which are informational, e.g. CPU utilization or memory utilization, but they are not critical for the service, I decided to keep the `/healthz` endpoint as a separate endpoint which returns the overall health status, so it can be used by monitoring tools unrelated to Kubernetes.
...
Being an advocate for open-source software, I decided that this package should be open-source and available for everyone to use under the name of [`github.com/kula-app/go-health`](https://pkg.go.dev/github.com/kula-app/go-health).
...
A small but important caveat: liveness probe failures cause Kubernetes to restart the pod, so `RegisterLivenessCheck` should be reserved for in-process signals that a restart can actually fix (deadlock detectors, goroutine-leak watchdogs, corrupt internal caches).
External dependencies belong on `RegisterReadinessCheck` instead, which only drains traffic from the affected replica rather than cascading restarts across the fleet.
